...
| 模块 | 状态 | 备注 |
|------|------|------|
| B 站单稿 / 多分 P 合集 | 可用 | `pagelist` 拉全分 P；未登录约 ≤720p |
...
| 合集勾选 / 批量下载 | 可用 | 至少 1 条；取消全选保留当前详情项；一次最多 5 个任务 |
| 导航 / 刷新 / 返回 | 可用 | `sessionStorage` + `history`；返回不中断下载 |
| 账号 / 支付 / 公网部署 | 未做 | 见 P1 |

文档：

- `docs/VideoHub产品需求文档.docx` — 原 PRD（含表格整理版）
- `docs/阶段性补充需求.md` — 增量需求与验收口径
- `README.md` — 对外启动与平台说明
...
- N 选下载 → 浏览器栏出现 N 个任务（上限 5），文件名可用（合集建议带分 P）。
...
若实现与本文冲突，以代码与 `docs/阶段性补充需求.md` 为准，并回写本文对应小节。
